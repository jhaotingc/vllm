...
class DFlashProposer(SpecDecodeBaseProposer):
    ...
    def build_per_group_and_layer_attn_metadata(
        ...
        # DFlash normally runs its drafter layers with non-causal attention.
        # ``dflash_causal`` (a config option kept from our build) flips the whole
        # drafter to causal attention; the base group metadata follows it.
        base_causal = self.dflash_causal
        ...
        for attn_group in self.draft_attn_groups:
            ...
            # forward context. Keep the base group metadata for group-level spec
            # decode checks, and specialize only the SWA layers that need a
            # causal sliding-window mask (when the base group is non-causal).
            causal_layers = sliding_layer_names & set(attn_group.layer_names)
            if causal_layers and not base_causal:
        ...
        for layer_name, attn_metadata in per_layer.items():
            ...
            if layer_name in sliding_layer_names and not base_causal:
                assert actual_causal is True, (
                    f"Attention metadata for sliding layer {layer_name} does not have"
                    " causal support, which is required for DFlash SWA."
                )
                continue
            assert actual_causal is base_causal, (
                f"Attention metadata for layer {layer_name} does not have the"
                f" {'causal' if base_causal else 'non-causal'} support required for"
                " DFlash."
